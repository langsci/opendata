...
\usepackage{url}
\usepackage{tikz}
\usetikzlibrary{arrows,positioning,shapes,quotes} 
\urlstyle{same}
...
\renewcommand{\lsImpressumCitationText}{
  ...
  Sebastian Nordhoff. %
  {\lsYear}. %
  \textit{Cookbook for Open Access books}. % 
  Berlin: Language Science Press.
}
\BackBody{This book describes the experiences of setting up a community-based publisher, Language Science Press.
...
This book is complemented by the business model (DOI: \url{https://doi.org/10.5281/zenodo.1286972}), open business data, and a spreadsheet for drafting and calculating own business models.
}

\renewcommand{\lsID}{cookbook}
\title{Cookbook for Open Access books}
\author{Sebastian Nordhoff\newlineCover Language Science Press}
\proofreader{Felix Kopecky, Jeroen van de Weijer}
\renewcommand{\lsISBNdigital}{978-3-96110-089-7}
\renewcommand{\lsISBNsoftcover}{978-1721065851}
\BookDOI{10.5281/zenodo.1286925}
\begin{document}
\maketitle 
\tableofcontents 
\mainmatter
...
Series editors should be aware of who in their field has book publication projects. In the case of \textit{Studies in Diversity Linguistics}, we were able to get a manuscript from a senior scholar which was already finished, but had never been formally published \citep{Dahl2016}.\footnote{\fullcite{Dahl2016}} Technically, that manuscript had been available from the document server of Stockholm university for some time. Since its formal publication with Language Science Press, that book has seen more than 3000 downloads. This is probably more than it would have gotten if it had only been available from that document server. For Language Science Press, however, more important than the downloads  was the fact that we could count Östen Dahl as one of the leading Scandinavian linguists among our authors. This complemented the list of otherwise junior authors for our first books and sent out the message that Language Science Press was to be considered as a serious venue for senior researchers as well. 

Typical careers in linguistics only give you the time during your PhD to complete a monograph. After that, the research becomes more granular and more focussed, and journal articles and papers in edited volumes take over. Edited volumes are great to build up a relation with more authors. At the same time, edited volumes are really tough and demand about 3-4 times more work than a monograph of the same length. The first edited volume we did was \textit{The Alor-Pantar languages} \citep{Klamer2014},\footnote{\fullcite{Klamer2014}} and it was definitely much more demanding than we had expected. We are pretty certain that the paper authors and the editors questioned the wisdom of their idea of publishing with Language Science Press more than once. However, in the end, they seem happy, and several of them have now become regular authors in edited volumes of ours (Klamer, Kratochvíl, Corbett). It might help that \textit{The Alor-Pantar languages} was a huge success as far as downloads are concerned, with more than 12\,000 downloads at the time of writing. 
...
\eabox{\label{ex:ingestiongeneration}
\begin{tikzpicture}
 \node[rectangle,draw] (ingestion) {ingestion};
 \node[rectangle,draw] (generation) [right=2cm of ingestion] {generation}; 
 
 \node[] (outlet2)  [right=of generation,text width=3cm] {\footnotesize Zenodo};
 \node[] (outlet1)  [above=2mm of outlet2,text width=3cm] {\footnotesize own website};
 \node[] (outlet3)  [below=2mm of outlet2,text width=3cm] {\footnotesize print-on-demand};
 
 \node[] (input2)  [left=of ingestion]  {\footnotesize doc};
 \node[] (input1)  [above=2mm of input2] {\footnotesize tex};
 \node[] (input3)  [below=2mm of input2] {\footnotesize odt};
 
 \draw[->] (input1) -- (ingestion) ;
 \draw[->] (input2) -- (ingestion) ;
 \draw[->] (input3) -- (ingestion) ;
 
 \draw[] (ingestion) edge ["\scriptsize\itshape conforming"](generation);
 
 \draw[->] (generation) -- (outlet1);
 \draw[->] (generation) -- (outlet2);
 \draw[->] (generation) -- (outlet3);
\end{tikzpicture}
}
...
For the initial period, we proposed complimentary conversion of docx and odt submissions (see \REF{ex:ingestiongeneration}). In the beginning, this held for any document. As we progressed, we created templates which are tailored to our conversion pipeline. We now require that authors use these templates if they want us to convert their manuscript for them. As the number of submission progresses, the press can become stricter in enforcing the formal requirements.  
...
% \todo{JvdW: I think this needs more info (in comparison with the technical part before): what is most important? how long does the planning take? tips/tricks, etc.}
...
The relations of the project to its home university can become a matter of concern as the publishing platform evolves. In the beginning, it will be only a project among many others, but as it progresses, it might outgrow the frame of an institute of a faculty. For instance, it would be strange if a smallish institute for Scandinavian Linguistics had three staff for publishing next to only 1 post-doc and 1 PhD student. In those cases, a transfer of the project to a larger structure (the university library or IT service), or to an independent company should be considered. See Section \ref{sec:legalform} on legal forms above. 
...
\sloppy
\begin{itemize}
\item \fullcite{Eve2014}
\item \fullcite{MaronEtAl2016} 
\item \fullcite{AdemaStone2017}
\item \fullcite{FewerdaEtAl2017}
\item \fullcite{JohsonEtAl2017}
\end{itemize}
\fussy
...
\chapter{Timeline}
The following is a rough timeline which has the official launch as reference point. There are a number of tasks	 which precede the official launch while others follow
...
\begin{itemize}
 ...
 \item[+1y] publish the majority of your invited titles. 
